...
class ReferenceLine extends Component {
  ...
  getEndPoints(scales, isFixedX, isFixedY, isSegment) {
    ...
    if (isFixedY) {
      ...
      if (
        ifOverflowMatches(this.props, "discard") &&
        !scales.y.isInRange(coord)
      ) {
        return null;
      }

      const points = [{ x: x + width, y: coord }, { x, y: coord }];
    ...
    if (isFixedX) {
      ...
      const points = [{ x: coord, y: y + height }, { x: coord, y }];
  ...
  render() {
    ...
    const scales = LabeledScaleHelper.create({
      x: xAxis.scale,
      y: yAxis.scale
    });
...
export default pureRender(ReferenceLine);
